...
export async function getRelaySwapData(contractAddress, sourceToken, destinationToken, amountIn) {
  // Call 1: probe for actual price impact (no slippage passed → Relay applies 1% default, ignored here)
  ...
  // Derive dynamic slippage from the observed impact so on-chain minAmountOut scales with pair/amount
  let priceImpact = null;
  if (probe.quote?.details?.swapImpact?.percent !== undefined && probe.quote?.details?.swapImpact?.percent !== null) {
    priceImpact = parseFloat(probe.quote.details.swapImpact.percent);
  } else if (probe.quote?.details?.totalImpact?.percent !== undefined && probe.quote?.details?.totalImpact?.percent !== null) {
  ...
  // Call 2: re-quote with computed slippage so the returned steps encode the right minAmountOut
