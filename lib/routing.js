...
export async function executeDCA(contract, buyer, amountPerDay, sourceToken, destinationToken) {
  try {
    ...
    // Extract price impact from Relay quote
    // Positive = profitable (you get more), Negative = unfavorable (you get less)
    ...
    } else if (relayData.quote?.steps && Array.isArray(relayData.quote.steps)) {
      // Fallback estimation based on number of hops (negative = cost)
      const numHops = relayData.quote.steps.length;
      priceImpact = -(numHops * 0.3);
    }

    // Extract slippage tolerance from Relay quote
    ...
    return {
      ...
      priceImpact: priceImpact, // Preserve sign
      slippagePercent: slippagePercent, // Slippage tolerance
    };
  } catch (error) {
