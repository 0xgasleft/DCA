...
// Errors where retrying is pointless — the failure is structural, not transient. Skip the
// remaining retry budget so we don't burn Relay quota on guaranteed-failure attempts.
const PERMANENT_ERROR_PATTERN = /AMOUNT_TOO_LOW/i;
  ...
  try {
    ...
    for (const session of sessions) {
      ...
      for (let attempt = 0; attempt <= RETRY_SCHEDULE.length; attempt++) {
        if (attempt > 0) {
          // Skip retries when the error is structural (e.g. AMOUNT_TOO_LOW). Retrying won't
          // change the answer and just burns Relay quota on guaranteed-failure attempts.
          if (lastErr && PERMANENT_ERROR_PATTERN.test(lastErr.message)) {
            console.log(`Skipping retries for ${session.address} -> ${session.destination_token}: permanent error (${lastErr.message})`);
            break;
          }
