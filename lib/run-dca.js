...
export default async function launchDCA(contract, sessions) {
  try {
    ...
    // Exponential backoff: 3 attempts total. Delays between attempts: 2s, 6s.
    // Tuned for Relay's typical transient window (NO_SWAP_ROUTES_FOUND, rate limits)
    // which usually clears within one block (~1-2s on Ink).
    ...
    for (const session of sessions) {
      ...
      for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
        if (attempt > 0) {
          ...
          console.log(`Retry ${attempt}/${RETRY_DELAYS_MS.length} for ${session.address} -> ${session.destination_token} after ${delay}ms...`);
          await new Promise(resolve => setTimeout(resolve, delay));
        }
        try {
          const result = await doDCA(contract, session);
          successCount++;
          results.push({ session: { address: session.address, destination_token: session.destination_token }, ...result, retried: attempt > 0, attempts: attempt + 1 });
          succeeded = true;
          break;
        } catch (err) {
          lastErr = err;
